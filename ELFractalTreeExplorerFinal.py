...
class FractalTree():
    '''Fractal Tree class that manages the drawing and storing of each fractalTree'''
    ...
    def generateFractalTreeManager(self):
        '''intial call to draw the fractal tree'''
        self.lines = []
    ...
    def generateFractalTree(self, levels, intialAngle, theta, p, length, scaleFactor, color):
        '''recursive function that creates the lines for each level of the fractal tree'''

        # I use the levels to determine what part of the tree is considered part of a leaf
        ...
        if levels < self.maxLevels:
            if self.probDraw > random():
                p1 = p.clone()
                # adds randomness to scale factor and theta using uniform function.
    ...
    def undrawFractalTree(self):
        '''undraws the lines of the fractal tree'''
        for l in self.lines:
            l.undraw()
...
def updateTextBoxes(ft):
    '''updates the textboxes on the gui'''
...
def redrawIfDrawn(ft):
    '''checks the fractal tree is drawn, if it is, undraw it and then redraw it'''
    if ft.isDrawn:
