...
class FractalTree():
    ...
    def generateFractalTree(self, levels, intialAngle, theta, p, length, scaleFactor):
        # base case
        self.lines.append(drawLine(self.win, p, intialAngle, length, lineThickness=self.lineThickness,
        ...
        if levels < self.maxLevels:
            if self.probDraw > random():
                ...
                self.generateFractalTree(levels + 1, t + intialAngle, t, p1, length * sf, sf)
            if self.probDraw > random():
                p2 = p.clone()
                sf = scaleFactor + uniform( -self.scaleFactorRandomness, self.scaleFactorRandomness)
                t = theta + uniform(-self.branchRandomness, self.branchRandomness)
                self.generateFractalTree(levels + 1, intialAngle - t, t, p2, length * sf, sf)
